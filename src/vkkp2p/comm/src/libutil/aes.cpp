...
void IC_AES::set_key(const u1byte in_key[], const u4byte key_len)
{   
	u4byte  i, t, u, v, w;
	if(!tab_gen)
		gen_tabs();
...
#define f_lround(bo, bi, k) \
    ...
    f_rl(bo, bi, 2, k);     \
f_rl(bo, bi, 3, k)
void IC_AES::encrypt(const u1byte in_blk[16], u1byte out_blk[16])
{   
	u4byte  b0[4], b1[4], *kp;
...
#define i_lround(bo, bi, k) \
    ...
    i_rl(bo, bi, 2, k);     \
i_rl(bo, bi, 3, k)
void IC_AES::decrypt(const u1byte in_blk[16], u1byte out_blk[16])
{
	u4byte  b0[4], b1[4], *kp;
