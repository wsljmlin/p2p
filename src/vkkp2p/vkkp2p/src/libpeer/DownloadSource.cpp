...
SourceInfo* DownloadSource::add_source(PTL_P2T_PeerInfo* source)
{
	...
	DOWNLOADSOURCE_PRT("add peer realip %s:%d, and localip %s:%d\n", Util::ip_htoas(source->tcpRealIP).c_str(), source->tcpRealPort,Util::ip_htoas(source->tcpLocalIP).c_str(), source->tcpLocalPort);
	return inf;
}
		...
		if(it2!=m_sources.end())
		{
			...
			else if(!data->isReady)
			{
				...
				if(it2->second->connectFailTimes>=2)
				{
					...
					delete it2->second;
					m_sources.erase(it2);
					DOWNLOADSOURCE_PRT("connect twice failed, will delete source!!!\n");
				}
			}
			else
			{
				...
				if(PS_NOTHING_TO_DOWNLOAD == data->state)
				{
					...
					delete it2->second;
					m_sources.erase(it2);
					DOWNLOADSOURCE_PRT("have not my needed data, will delete source!!!!\n");
				}
			}
		}
	}
...
int DownloadSource::create_channel_i(SourceInfo& inf)
{
	...
	if(peer)
	{
		...
		DOWNLOADSOURCE_PRT("create peer realip %s:%d, and localip %s:%d\n",  Util::ip_htoas(inf.source.tcpRealIP).c_str(), inf.source.tcpRealPort,Util::ip_htoas(inf.source.tcpLocalIP).c_str(),inf.source.tcpLocalPort);		
...
#ifdef SM_VOD
		TrackerSngl::instance()->PTL_ReportShareFile(m_fi->hash, (int)m_playtype);
