...
void LogManager::update_fd()
{
	...
	if(m_curr_date != curr_date)
	{
...
#ifdef SM_MODIFY
		sprintf(_tmpbuf,"\r\n<br>[%s]:uid,user_type,ip,str_hash,hash_type,programetype\r\n",_CURR_TIME_STR);
#else
		...
		sprintf(_tmpbuf,"\r\n<br>[%s]:flag,      uid,     nattype,	 ip,      str_hash, "
				"   hash_type(programetype, filesize, speed_KB, downseconds, cacheTimes, dragTimes, cacheSeconds), "
...
#ifdef SM_MODIFY
	sprintf(_tmpbuf,"<br>[%s]: '%s' ,%5d, '%s',  '%s',%5d, %d \r\n",_CURR_TIME_STR,inf->uid,inf->user_type,Util::ip_htoas(inf->ip).c_str(),inf->str_hash,inf->hash_type,inf->filetype);
#else
...
void LogManager::on_downloadfileinfo(DBDownloadFileInfo *inf)
{
...
#ifdef SM_MODIFY
#ifdef SM_VOD
	int playtype = SourceServiceSngl::instance()->get_sourcetype(_tmphash);
#else
	int playtype = 0;
#endif /* end of SM_VOD */
#endif /* end of SM_MODIFY */
	...
	sprintf(_tmpbuf,"<br>[%s]:%d, '%s' , %d, '%s' , '%s', "
		"   %d(%d,%10lld,%6d KB, %d S, %d, %d, %d), "
		"   c-s/f(%d/%d, %d/%d, %d/%d, %d/%d, %d/%d),"
		"   s-KB(%d, %d),"
		"   d-IPT-KB(%d, %d, %d),"
		"   d-UT-KB(%d, %d, %d, %d, %d) \r\n",
		_CURR_TIME_STR,(int)inf->df.flag,inf->uid,inf->nattype,Util::ip_htoas(inf->ip).c_str(),_tmpstrhash,
		(int)_tmphash.hash_type(),playtype,df.size,df.speed_KB,df.downSeconds,df.cacheTimes,df.dragTimes,df.cacheSenconds,
#else
