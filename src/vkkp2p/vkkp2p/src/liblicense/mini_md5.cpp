...
// 
// md5.cpp  change the namespace to miniutil
//
//                                 wangbo
//
//
...
MD5::MD5(unsigned char *string, int len ) {
  init();  // must be called be all constructors
  unsigned int nlen = ( len > 0 ) ? (unsigned int) len  : strlen((char*)string);
  update( string ,  nlen );
  finalize ();
}
